...
public class episode {
    ...
    public void runAlgo(int window_size, double min_sup) throws FileNotFoundException {
        ...
        try {
            ...
            for (ArrayList<ArrayList<String>> F1_with_target : F1_with_targets) {
                ...
                SerialJoin(F1_with_target, lastitem, F1, CES, min_sup, window_size, order);
     ...
     *   
     */  
    private void SerialJoin(ArrayList<ArrayList<String>> alpha, ArrayList<ArrayList<String>> lastItem, ArrayList<ArrayList<ArrayList<String>>> F1, ArrayList<ArrayList<String>> CES, double min_sup, int window_size, HashMap<ArrayList<ArrayList<String>>, Integer> order) {
        for (ArrayList<ArrayList<String>> f_j : F1) {
            ...
            if (order.get(f_j) >  order.get(lastItem)) {
                ...
                if (sup >= min_sup) {
                    ...
                    try {
                        saveEpisode(equal_join, sup, conf);
                    } catch (IOException e) {
                        System.out.println("IOException!");
                    }
                    SerialJoin(equal_join, f_j, F1, CES, min_sup, window_size,  order);
                }
            } 
            ...
            //        System.out.println("temp: "  + temporal_join);
                    try {
                        saveEpisode(temporal_join, sup, conf);
                    } catch (IOException e) {
                        System.out.println("IOException!");
                    }
                    SerialJoin(temporal_join, f_j, F1, CES, min_sup, window_size,  order);
            }
